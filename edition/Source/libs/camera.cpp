...
*/
Camera::Camera(const Vector& eye, const Vector& at, const Vector& up, const double& width, const double& height, const double& near, const double& far) :eye(eye), at(at), up(up), width(width), height(height)
{
...
/*!
\brief Rotates the camera with a rotation matrix.
...
{
  if ((box.Center() - eye) * view < -box.Radius())
  {
    return false;
  }
  return true;
