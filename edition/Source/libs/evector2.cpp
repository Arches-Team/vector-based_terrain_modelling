...
*/
QString Vector2::ToString(int n) const
{
  return QString("Vector2(%1,%2)").arg(c[0], 0, 'f', n).arg(c[1], 0, 'f', n);
}
...
{
  n = n.replace("Vector2(", "").replace(')', "");
  QStringList cmps = n.split(',');
  return Vector2(cmps[0].toDouble(), cmps[1].toDouble());
}
...
/*!
\brief Return QString of the form 'vec2(x, y)' that write the vector in GLSL.
\author Hubert-Brierre Pierre

\param u %Vector.
*/
QString ToGLSL(const Vector2& u)
{
  return "vec2(" + QString::number(float(u[0])) + ", " + QString::number(float(u[1])) + ")";
}
