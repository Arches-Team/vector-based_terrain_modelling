...
double Vector::Angle(const Vector& b) const
{
  ...
  //angle = ((*this / b)[2] < 0.0) ? Math::TwoPi - angle : angle;
  return angle;
}
...
void Vector::RandomOrthonormal(Vector& i, Vector& j) const
{
  ...
  double r = Random::R239.Uniform(Math::TwoPi);
  double c = cos(r);
  double s = sin(r);
...
/*!
\brief Create a QString form a vector.
...
*/
QString Vector::ToString(int n) const
{
  return QString("Vector(%1,%2,%3)").arg(c[0], 0, 'f', n).arg(c[1], 0, 'f', n).arg(c[2], 0, 'f', n);
}
...
/*!
\brief Creates a vector from a QString.

\param n String.
*/
Vector Vector::FromString(QString n)
{
  n = n.replace("Vector(", "").replace(')', "");
  QStringList cmps = n.split(',');
  return Vector(cmps[0].toDouble(), cmps[1].toDouble(), cmps[2].toDouble());
}
...
/*!
\brief Return QString of the form 'vec3(x, y, z)' that write the vector in GLSL.
\author Hubert-Brierre Pierre

\param u %Vector.
*/
QString ToGLSL(const Vector& u)
{
  return "vec3(" + QString::number(float(u[0])) + ", " + QString::number(float(u[1])) + ", " + QString::number(float(u[2])) + ")";
}
