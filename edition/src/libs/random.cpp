...
/*!
\brief Exponential random sequence.

Give a random variate drawn with a uniform distribution T=-ln(1-U)/lamba has
an exponential distribution. Since U is uniform, 1-U and U are equivalent,
thus T=-ln(U)/lambda.

\param mu Mu parameter, invese of lamba.
*/
...
/*!
\brief Normal distribution using a Box-Muller algorithm.

The Boxâ€“Muller transform is a pseudo-random number sampling method for
generating pairs of independent, standard, normally distributed (zero
expectation, unit variance) random numbers, given a source of uniformly
distributed random numbers.

\param variance Variance.
\param mean Mean.
*/
double Random::NormalBoxMuller(const double& variance, const double& mean)
{
  double r;
  double x;
  double y;
  ...
  double un1 = sqrt(-2 * log(r) / r) * x;
  //double un2 = sqrt(-2 * log(sum) / sum) * y;

  double sqrt_variance = sqrt(variance);
  double n1 = mean + sqrt_variance * un1;
  //double n2 = mean + sqrt_variance * un2;

  return n1;
}

/*!
\brief Generate a random angle.
*/
double Random::Angle(unsigned int var)
{
  return NormalBoxMuller(var, 0);
}
