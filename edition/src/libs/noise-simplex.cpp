...
/*!
\class SimplexNoise noise.h
\brief Simplex noise.

\brief Simplex noise returns random values in [-1,+1] interval.

\sa Noise
*/

// Gradients
const Vector SimplexNoise::gradient[12] = {
...
/*!
\brief Compute the simplex noise at a given point.
\param p Point.
*/
double SimplexNoise::Value(const Vector& p) const
{
  ...
  if (t0 >= 0) {
    t0 *= t0;
    n[0] = t0 * t0 * (gradient[gi0] * Vector(x0, y0, z0));
  }
  ...
  {
    t1 *= t1;
    n[1] = t1 * t1 * (gradient[gi1] * Vector(x1, y1, z1));
  }
  ...
  {
    t2 *= t2;
    n[2] = t2 * t2 * (gradient[gi2] * Vector(x2, y2, z2));
  }
  ...
  {
    t3 *= t3;
    n[3] = t3 * t3 * (gradient[gi3] * Vector(x3, y3, z3));
  }
