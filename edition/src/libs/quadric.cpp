...
/*!
\class Quadric quadric.h
\brief %Quadric polynomials.

Constructors should provide the coefficients in descending order.
 is an example of how to code the
quadric 2 x<SUP>2</SUP>-x+1:
\code
...
{
  s << "Quadric(" << p[2] << ',' << p[1] << ',' << p[0] << ')';
  return s;
}

/*!
\brief Creates a quadratic Bezier polynomial on interval [0,1].
\param a,b,c The parameters of the B̩zier polynomial.
...
{
  if (p == 0)
    return Quadric(1.0, -2.0, 1.0);
  else if (p == 1)
    return Quadric(-2.0, 2.0, 0.0);
  else // if (p == 2)
    return Quadric(1.0, 0.0, 0.0);
}
...
\param x Real.
*/
double Quadric::Bernstein(int p, const double& x)
{
  if (p == 0)
...
/*!
\brief Compute the Lipschitz constant of the quadric.

\param a,b Interval.
*/
double Quadric::K(const double& a, const double& b) const
{
  double x, y;

  Linear linear = Prime();
  linear.Range(x, y, a, b);
  return Math::Max(fabs(x), fabs(y));
}

/*!
\brief Create a quadric from a set of roots.
\param r Set of roots.

Example of how to code the quadric (x-2)(x+1):
\code
Quintic p=Quintic::FromRoots({2.0,1.0});
\endcode
*/
