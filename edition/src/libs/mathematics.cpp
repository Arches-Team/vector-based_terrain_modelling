...
{
  return sin(x - floor(x) * Math::TwoPi);
}
...
/*!
\brief Fold the argument angle to [0,2\pi[ or [-\pi;pi[
\param a Angle.
\param s Strategy, map to [0,2\pi[ if true, [-\pi;pi[ otherwise.
*/
double Math::FoldAngle(const double& a, bool s)
{
  // Map to [-Math::TwoPi,Math::TowPi]
  double y = fmod(a, Math::TwoPi);
...
/*!
\brief Trigonometric simplification of sin(atan(x)).
\param x Real value.
*/
...
/*!
\brief Trigonometric simplification of cos(atan(x)).
\param x Real value.
*/
...
double FastAtan(double x)
{
  double y = fabs(x);
  double a = 0.0;
  // Simply because tan^-1 (x) + tan^-1 (1 / x) = pi / 2 if x > 0
  if (y > 1.0)
  {
    // a= Math::HalfPi - FastAtan(1.0 / y); // Avoid recursion
    y = 1.0 / y;
    a = Math::HalfPi - y / (1.0 + 0.28086 * y * y);
  }
  else
  {
    a = x / (1.0 + 0.28086 * x * x);
...
\param A %Array of five reals.
*/
void Math::Sort5(double* A)
{
  // Sort {A[0], A[1]} and {A[2], A[3]} with 1 <.
  Math::Sort(A[0], A[1]);
  Math::Sort(A[2], A[3]);

  // Sort {A[0], A[1]} and {A[2], A[3]} by the first item with 1 <: A[0], A[1], A[2], A[3] -> A[2], A[3], A[0], A[1]
  if (A[0] > A[2])
  {
    Math::Swap(A[0], A[2]);
    Math::Swap(A[1], A[3]);
  ...
  // Sort A[4] into {A[0], A[2], A[3]} with 2 <.
  if (A[4] > A[2])
  {
    if (A[4] < A[3]) // A[3], A[4] -> A[4], A[3]
    {
      Math::Swap(A[3], A[4]);
    }
    else
    {
      if (A[4] > A[0]) // A[2], A[3], A[4] -> A[4], A[2], A[3]
      {
        double t = A[2];
        A[2] = A[4];
        A[4] = A[3];
      ...
      else // A[0], A[2], A[3], A[4] -> A[4], A[0], A[2], A[3]
      {
        double t = A[0];
        A[0] = A[4];
        A[4] = A[3];
  ...
  // Sort A[1] into {A[2], A[3], A[4]} with 2 <.
  if (A[1] > A[3])
  {
    if (A[1] > A[4]) // A[1], A[2], A[3], A[4] -> A[2], A[3], A[4], A[1]
    {
      double t = A[1];
      A[1] = A[2];
      A[2] = A[3];
